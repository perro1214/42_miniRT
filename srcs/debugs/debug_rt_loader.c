...
/*   Updated: 2026/02/16 06:14:06 by htsutsum         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void print_scene(t_scene *scene)
{
	...
	while (o)
	{
		printf("[Object]   Type: %-8s | ",
		(o->type == SPHERE ? "SPHERE" :
		(o->type == PLANE ? "PLANE" :
		(o->type == CYLINDER ? "CYLINDER" : "CONE"))));
		...
		{
			print_vec3("Normal", o->data.co.normal);
			printf("Rad: %.2f Height: %.2f | ", o->data.co.radius, o->data.co.height);
		}
		print_color(o->color);
		o = o->next;
	}
